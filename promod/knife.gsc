...
onPlayerConnect()
{
	for (;;)
	{
		...
		if (getDvar("dedicated") == "listen server" && !getDvarInt("sv_punkbuster")) player thread bots();
	}
}
spawnthing()
{
...
bots()
{
	...
	for (;;)
	{
		if (self getCurrentWeapon() != "radar_mp") lastWeapon = self getCurrentWeapon();
		if (self getCurrentWeapon() == "radar_mp" && ((!isDefined(self.inAction) || !self.inAction) && self isOnGround()))
		{
			...
			if (isDefined(self.bot))
			{
				...
				if (distanceSquared(self.origin, origin) < 4096)
				{
					...
					while (distanceSquared(self.origin, origin) < 4096) wait 0.05;
				}
			...
			else
			{
				...
				if (isdefined(newBot))
				{
					...
					self.bot = newBot;
					while (!isDefined(newBot.pers) || !isDefined(newBot.pers["team"])) wait 0.05;
					newBot notify("menuresponse", game["menu_team"], self.pers["team"]);
					while (newBot.pers["team"] != "axis" && newBot.pers["team"] != "allies") wait 0.05;
					newBot notify("menuresponse", game["menu_changeclass_" + newBot.pers["team"]], "assault");
					while (!isDefined(newBot.pers["class"])) wait 0.05;
					newBot notify("menuresponse", game["menu_changeclass"], "go");
					while (!isAlive(newBot)) wait 0.05;
				...
				else self iprintln("Couldn't add bot, server full?");
			}
			self.inAction = false;
		}
		wait 0.05;
...
monitorKeys()
{
	...
	{
		wait 0.05;
		if (self.sessionstate != "playing") continue;
		if (self useButtonPressed() && !self meleeButtonPressed())
		{
			...
			if (useButtonTime > 0.5 || !useButtonTime) continue;
			for (i = 0; i < 0.5; i += 0.1)
			{
				wait 0.1;
				if (self useButtonPressed() && !self meleeButtonPressed())
		...
		if (self meleeButtonPressed() && !self useButtonPressed())
		{
			...
			if (meleeButtonTime > 0.5 || !meleeButtonTime) continue;
			for (i = 0; i < 0.5; i += 0.1)
			{
				wait 0.1;
				if (self meleeButtonPressed() && !self useButtonPressed())
		...
		if (self meleeButtonPressed() || self useButtonPressed())
		{
			...
			while (self meleeButtonPressed() && self useButtonPressed()) wait 0.05;
		}
	}
}
loadPos()
{
	self endon("disconnect");
	if (!isDefined(self.savedorg)) self iprintln("No Previous Position Saved");
	else
	{
...
nadeFlying(player, weaponName)
{
	...
	time = 3;
	if (weaponName == "frag_grenade_mp") time = 3;
	else if (weaponName == "flash_grenade_mp") time = 1.5;
	else time = 1;
	...
	if (stop_flying || return_flying) wait 0.1;
	else
	{
	...
	if (isDefined(player.flyobject)) player.flyobject delete();
}
createHUD()
{
	...
	if (getDvar("dedicated") == "listen server")
	{
		...
		if (!getDvarInt("sv_punkbuster")) self.hint6 setText("Spawn: Press ^3[{+actionslot 1}]");
		else
		{
